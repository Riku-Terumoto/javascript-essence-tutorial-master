...
function minus(obj, val) {
  ...
  //  prop1はコピーなので全くの別物になり、そのままprop1に代入してもobjの値は変わらない
  //  prop1 = prop1 - val;
  obj.prop1 = prop1 - val;
}
...
function fn({ prop2 }) {
  let prop = prop2; // {prop3:1}
  prop.prop3 = 2; // {prop3:2}に更新
  prop = { prop3: 3 }; // {prop3:3}更新
  return { prop2: prop }; // prop2 = {prop3:3}
}
obj = fn(obj);
...
// 等価である
// objは引数に渡し、帰ってくるまで何も処理されていないので、参照元は変わらずtrueとなる
